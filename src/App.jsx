...
function Contact() {
  ...
  const contacts = [
    ...
    {icon:"🌐",label:"Portfolio",val:"subathrafsdportfolio.netlify.app",href:"https://subathrafsdportfolio.netlify.app"},
  ...
  return (
    <section id="contact" className="px-8 py-16 bg-[#0a0a0f]">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-slate-400 mb-8 max-w-lg text-sm leading-relaxed">I'm open to full-time roles, internships, and freelance projects. Don't hesitate to reach out!</p>
        <div className="reveal grid grid-cols-2 md:grid-cols-3 gap-4">
          {contacts.map(c=>{
            const inner = (
              <div className="bg-[#1a1a26] border border-violet-900/20 rounded-2xl p-5 flex flex-col items-center text-center gap-2 hover:-translate-y-1 hover:border-violet-500/40 hover:shadow-xl hover:shadow-violet-500/10 transition-all duration-200 h-full">
                <span className="text-3xl">{c.icon}</span>
                <span className="text-xs font-bold uppercase tracking-widest text-slate-500">{c.label}</span>
                <span className="text-sm font-medium text-white break-all">{c.val}</span>
              </div>
            );
            return c.href ? (
              <a key={c.label} href={c.href} target={c.href.startsWith("http")?"_blank":undefined} rel="noopener noreferrer" className="no-underline">{inner}</a>
            ) : <div key={c.label}>{inner}</div>;
          })}
        </div>
        <div className="flex justify-center mt-8">
          <a href="https://drive.google.com/file/d/1ys15wRbrtzhHpKA2QuGWOuQPjIXAKfnw/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="px-8 py-4 rounded-full bg-gradient-to-r from-violet-500 to-violet-600 text-white font-semibold text-sm no-underline hover:-translate-y-0.5 hover:shadow-lg hover:shadow-violet-500/40 transition-all duration-200 shadow-md shadow-violet-500/20">
            📄 Download My Resume
          </a>
        </div>
      </div>
...
function BackTop() {
  ...
  if (!show) return null;
  return (
    <button onClick={() => document.getElementById("home")?.scrollIntoView({behavior:"smooth"})} className="fixed bottom-6 right-6 w-11 h-11 rounded-full bg-violet-500 text-white flex items-center justify-center text-lg shadow-lg shadow-violet-500/40 hover:-translate-y-1 hover:bg-violet-400 transition-all duration-200 z-50 border-none cursor-pointer">
      ↑
    </button>
  );
}
